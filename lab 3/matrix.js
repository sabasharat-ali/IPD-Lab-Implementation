...
function rowcol() {
  ...
  if (row1 < 2 || col1 < 2 || row1 > 5 || col1 > 5) {
    document.getElementById("rowcol").innerHTML =
      "rows and colums cannot be greater than 5 or less than 2";
  } else {
    for (let i = 1; i <= row1; i++) {
      for (let j = 1; j <= col1; j++) {
        ...
        node.innerHTML =
          ...
          " column " +
          j +
          ': </label><input type="number" id="row' +
          i +
          "col" +
          j +
          'val"><br>';
...
function setmatrices() {
  //saving elements of matrix in array and showing operations
  for (let k = 1; k <= row1; k++) {
    for (let l = 1; l <= col1; l++) {
      console.log(document.getElementById("row" + k + "col" + l + "val").value);
      arr1.push(
        parseInt(document.getElementById("row" + k + "col" + l + "val").value)
      );
    }
  }
  ...
  document.getElementById("matopr").innerHTML =
    "Your matrix is " + mat1 + "<br><br>";
  var options = document.createElement("p");
  options.innerHTML =
    '<span onclick="deter()">Find Determinent</span><br><span onclick="transpose()">Find Transpose</span><br><span onclick="inverse()">Find Inverse</span><br><span onclick="addscalor()">Add Scalor</span><br><span onclick="multiplyscalor()">Multiply a scalor</span><br>';
  document.getElementById("matopr").appendChild(options);
}

function deter() {
  //function to find determinent
  if (row1 != col1) {
...
function transpose() {
  ...
  transResult.innerHTML =
    "Transpose of given Matrix is " + math.matrix(transpose1);
...
function inverse() {
  ...
  if (row1 != col1) {
    ...
    inverseResult.innerHTML =
      "Inverse of given Matrix is " + math.matrix(inverse1);
    document.getElementById("matopr").appendChild(inverseResult);
  }
}
function addscalor() {
  //function to get scalor to add to matrix
  var addsc = document.createElement("p");
  addsc.innerHTML =
    '<label for="addsc">Enter scalor number to add to the matrix: </label><input type="number" name="" id="addsc"><span onclick="sumscalor()">OK</span>';
...
function sumscalor() {
  ...
  sumsc.innerHTML =
    "Sum is " +
    math.matrix(
      math.add(mat1._data, parseInt(document.getElementById("addsc").value))
    );
  document.getElementById("matopr").appendChild(sumsc);
}
function multiplyscalor() {
  //function to get scalor to multiply to matrix
  var mulsc = document.createElement("p");
  mulsc.innerHTML =
    '<label for="mulsc">Enter scalor number to Multiply to the matrix: </label><input type="number" name="" id="mulsc"><span onclick="mulscalor()">OK</span>';
...
function mulscalor() {
  ...
  mulsc.innerHTML =
    ...
    math.matrix(
      math.multiply(
        mat1._data,
        parseInt(document.getElementById("mulsc").value)
      )
    );
  document.getElementById("matopr").appendChild(mulsc);
}
...
function rowcol1() {
  //for getting rows and columns of 2 matrices
  row1 = document.getElementById("row1").value;
...
function setmatrices2() {
  for (let k = 1; k <= row1; k++) {
    for (let l = 1; l <= col1; l++) {
      console.log(
        document.getElementById("row" + k + "col" + l + "valmat1").value
      );
      arr1.push(
        parseInt(
          document.getElementById("row" + k + "col" + l + "valmat1").value
  ...
  for (let k = 1; k <= row2; k++) {
    for (let l = 1; l <= col2; l++) {
      console.log(
        document.getElementById("row" + k + "col" + l + "valmat2").value
      );
      arr2.push(
        parseInt(
          document.getElementById("row" + k + "col" + l + "valmat2").value
...
function mulmat2mat1() {
  //function to perform Multiplication of matrix 2 with matrix 1
...
function submat1mat2() {
  //function to perform Subtraction of matrix 2 from matrix 1
...
function submat2mat1() {
  //function to perform Subtraction of matrix 1 from matrix 2
